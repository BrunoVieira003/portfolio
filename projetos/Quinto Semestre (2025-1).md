...
### 🗣️ Soft Skills
Como desenvolvedor, mantive uma comunicação constante com o time para alinhar as integrações entre backend e mobile, garantindo que as rotas, regras de negócio e retornos da API estivessem claros e consistentes. Fui proativo ao propor e implementar melhorias na experiência do usuário no aplicativo mobile, como a funcionalidade de atualização automática das listagens (refresh), a criação de filtros de pesquisa para facilitar a visualização das solicitações de reembolso e correções de layout ocasionados pelo uso de emojis na versão mobile. Além disso, exerci autonomia ao desenvolver e testar funcionalidades no backend e no app, assegurando a entrega de um sistema estável, intuitivo e alinhado às necessidades do usuário.
